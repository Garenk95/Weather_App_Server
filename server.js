...
async function getLocationKey(param, type) {
    ...
    if (type === "zip") {
        //url for zip code
        url = url + `/locations/v1/postalcodes/search?apikey=${apiKey}&q=${param}&language=en-us&details=false`;
    } else if (type === "ip") {
        //url for ip address
        url = url + `/locations/v1/cities/ipaddress?apikey=${apiKey}&q=${param}&language=en-us&details=false`;
    } else {
...
async function getCurrentConditions(locationKey) {
    console.log(`Fetching current conditions for location key (2): ${locationKey}`);
    const promise = new Promise((resolve, reject) => {
        fetch(`http://dataservice.accuweather.com/currentconditions/v1/${locationKey}?apikey=${apiKey}&language=en-us&details=false`)
            .then(response => response.text())
            .then(result => {
                var obj = JSON.parse(result);
                console.log(`Retrieved current conditions for location key (3): ${locationKey}`);
                console.log(obj);
                resolve(obj);
            });
    });
...
async function getFiveDayForecast(locationKey) {
    const promise = new Promise((resolve, reject) => {
        fetch(`http://dataservice.accuweather.com/forecasts/v1/daily/5day/${locationKey}?apikey=${apiKey}&language=en-us&details=false&metric=false`)
            .then(response => response.text())
            .then(result => {
                var obj = JSON.parse(result);
                console.log(`Retrieved 5 day forecast for location key(8): ${locationKey}`);
                resolve(obj);
            });
...
app.get(`/current_conditions_with_IP/:ip`, async (req, res) => {
    const locationKey = await weather_database.location_key_exists(req.params.ip) ? await weather_database.get_location_key(req.params.ip) : await getLocationKey(req.params.ip, "ip");
